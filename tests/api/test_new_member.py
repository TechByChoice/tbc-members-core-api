...
class CreateNewMemberTestCase(TestCase):
    def setUp(self):
        ...
        self.request = self.factory.patch(
            "/api/member", {"data": "test_data"}, format="json"
        )
    ...
    def test_create_new_member_success(
        self,
        mock_send_invite,
        mock_create_or_update_user_profile,
        mock_create_or_update_talent_profile,
        mock_create_or_update_user,
        mock_extract_talent_data,
        mock_extract_profile_data,
        mock_extract_company_data,
        ...
        mock_extract_company_data.return_value = {
            "company_name": "TestCompany",
            "company_url": "https://testcompany.com",
        }
        mock_extract_profile_data.return_value = {
            "linkedin": "https://linkedin.com/in/test"
        }
        mock_extract_talent_data.return_value = {
            "tech_journey": "5 years",
            "is_talent_status": True,
        }
...
class CreateUserTestCase(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create(
            username="testuser", email=self.user_data["email"]
        )
    ...
    def test_create_or_update_user_success(self, mock_user_serializer):
        ...
        # Assertions
        mock_user_serializer.assert_called_once_with(
            instance=self.user, data=self.user_data, partial=True
        )
...
class CreateUserProfileTestCase(TestCase):
    ...
    def test_create_or_update_user_profile_success(self, mock_get_or_create):
        ...
        mock_get_or_create.assert_called_once_with(
            user=self.user, defaults=self.profile_data
        )
