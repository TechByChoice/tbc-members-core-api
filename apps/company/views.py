...
def get_closest_match(commitment):
    ...
    for choice in JOB_TYPE_CHOICE:
        if normalize_string(choice[1]) in normalized_commitment or normalized_commitment in normalize_string(choice[1]):
            return choice[1]
    return None
...
def sync_jobs_from_lever(company):
    ...
    for job in tree.findall('job'):
        ...
        company_profile_instance, _ = company_profile, created = CompanyProfile.objects.get(
            company_name=job.find('employer').text
        )
...
def sync_jobs_from_workable():
    ...
    response = requests.get(WORKABLE_API_ENDPOINT, headers={"Authorization": "Bearer YOUR_API_KEY"})
...
def sync_jobs_from_greenhouse(company):
    ...
    headers = {
        "Authorization": f"Bearer {company.greenhouse_api_key}"
    }
    ...
    for job in jobs:
        ...
        job_instance, created = Job.objects.update_or_create(
            greenhouse_id=job_data['greenhouse_id'],
            defaults=job_data
        )
        if created:
            job_instance.department.set([department_instance])
...
def submit_candidate_to_greenhouse(talent_profile, api_key):
    ...
    try:
        ...
        if response.status_code == 201:
            return response.json()  # Return the response, which will typically include the candidate's new ID
        else:
...
class AbstractSetAPIKey(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        if not self.api_key_field:
            return Response({"detail": "API key field not set."}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
